...
UAuraAttributeSet::UAuraAttributeSet()
{
	...
	/**
	 * here we store a map with the tag and its function pointer thats why there is no () for the functions
	 */
	//primary
...
void UAuraAttributeSet::HandleIncomingDamage(const FEffectProperties& Props)
{
	...
	SetIncomingDamage(0.f);//this is for removing previous incoming damage , to prevent stacking 
	if (LocalIncomingDamage>0.f)
	{
		...
		if (UAuraAbilitySystemLibrary::IsSuccessfullDebuff(Props.EffectContextHandle))
		{
				Debuff(Props);
		}
	}
